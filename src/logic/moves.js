...
export function chooseShipments(G, ctx, selections) {
  ...
  if (
    ...
    (Object.keys(ctx.activePlayers).length === 1 &&
      ctx.playerID in ctx.activePlayers)
  ) {
    ctx.events.endTurn();
  }
}
...
export function canClaimRoute(selections, route, numChickens) {
  ...
  if (
    route.playerID !== null ||
    numChickens < route.length ||
    numCards !== route.length
  ) {
    return false;
  }
  ...
  return route.color === colors.GRAY
    ...
    : (keys.length === 1 && (hasMulti || hasColor)) ||
        (keys.length === 2 && hasMulti && hasColor);
}
